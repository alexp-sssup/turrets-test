 ...
 * cube faces are camera-facing, which face a placement builds across -- falls out of
...
export class ViewYaw {
   ...
   *
   * The x component is +/-1 at every yaw, so the traversal spacing is exactly one on every
   * axis and the pick is integer-driven with no epsilon (spec 5.2).
   */
